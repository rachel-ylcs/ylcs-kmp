...
@WorkerThread
expect fun compressFixed(image: Image, sink: BufferedSink)

@WorkerThread
expect fun decodeFixed(requestContext: RequestContext, imageInfo: ImageInfo, dataSource: DataSource): Image

expect fun supportImageFixed(image: Image): Boolean

class FixedResultCacheInterceptor : Interceptor {
    companion object {
    ...
    private fun readCache(requestContext: RequestContext): ImageData? {
        ...
        return runCatching {
            resultCache.openSnapshot(resultCacheKey)?.use { snapshot ->
                ...
                val image = decodeFixed(requestContext, metadata.imageInfo, dataSource)
                ImageData(
                    image = image,
    ...
    private fun writeCache(requestContext: RequestContext, imageData: ImageData): Boolean {
        ...
        if (!supportImageFixed(image)) return false
        val resultCache = requestContext.sketch.resultCache
        val resultCacheKey = requestContext.resultCacheKey
        val editor = resultCache.openEditor(resultCacheKey) ?: return false
        val result = runCatching {
            resultCache.fileSystem.sink(editor.data).buffer().use { compressFixed(image, it) }
