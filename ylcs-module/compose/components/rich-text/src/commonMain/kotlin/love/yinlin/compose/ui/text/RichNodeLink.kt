...
@Stable
class RichNodeLink internal constructor(
    val uri: String,
    val text: String,
    val color: Color?,
) : RichValue() {
    ...
    data object Drawer : RichDrawer {
        ...
        override fun RichRenderScope.render(item: RichObject) = item.cast<RichNodeLink> {
            builder.append(' ')
            builder.withLink(LinkAnnotation.Url(
                url = item.uri,
                styles = TextLinkStyles(style = SpanStyle(color = item.color ?: Colors.Yellow6)),
...
fun RichList.link(uri: String, text: String, color: Color? = null) = addNode(RichNodeLink(uri, text, color))