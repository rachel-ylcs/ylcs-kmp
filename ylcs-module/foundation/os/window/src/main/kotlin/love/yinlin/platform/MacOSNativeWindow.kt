...
internal object MacOSNativeWindow : NativeWindowImpl, NativeLibrary("/usr/lib/libobjc.A.dylib") {
    ...
    val objc_msgSend by func(
        NativeType.Pointer,
        NativeType.Pointer,
        NativeType.Boolean
    )

    val objc_autoreleasePoolPush by func(
        retType = NativeType.Pointer
    )

    val objc_autoreleasePoolPop by func(
        NativeType.Pointer
    )
    ...
    override fun updateClickThrough(handle: Long, enabled: Boolean) {
        ...
        try {
            val windowPtr = Native.Pointer.ofAddress(handle)
            useMemory { arena ->
                val selName = arena.allocateFrom("setIgnoresMouseEvents:")
                val sel = sel_registerName(selName) as Native.Pointer
