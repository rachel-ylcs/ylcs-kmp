...
object ModFactory {
    ...
    class Merge(
        ...
        suspend fun process(
            filter: List<ModResourceType> = listOf(),
    ...
    class Release(
        ...
        private suspend fun Source.readResource(mediaPath: Path) = Coroutines.io {
            ...
            require(ModResourceType.fromInt(resType) != null) { "不支持的资源类型 $resType" }
    ...
    class Preview(
        ...
        private suspend fun Source.previewResource(): Pair<ResourceItem, MediaConfig?> = Coroutines.io {
            val resType = readInt() // 读资源类型
            val resourceType = ModResourceType.fromInt(resType)
            ...
            require(resLength > 0) { "资源长度非法 Length: $resLength" }
            val times = resLength / INTERVAL
            val config = if (resourceType == ModResourceType.Config) { // 读取媒体配置
                val bytes = ByteArray(resLength)
                readTo(bytes)
                bytes.decodeToString().parseJsonValue<MediaConfig>()
            }
            else { // 跳过资源数据
        ...
        private suspend fun Source.previewMedia(): MediaItem = Coroutines.io {
            ...
            var mainConfig: MediaConfig? = null
            repeat(resourceNum) {
