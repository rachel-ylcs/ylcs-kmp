...
import love.yinlin.cs.SocketsManager
...
class LyricsSocketsManager(
    private val mysql: MysqlService,
    @Suppress("PrivatePropertyName") private val AN: Authorization,
    session: Any
    ...
    companion object {
        ...
        private val players = mutableMapOf<Int, Player>() // TODO: 需要修改成线程安全的哈希表
    ...
    private class Room(val info1: LyricsSockets.PlayerInfo, val info2: LyricsSockets.PlayerInfo) {
        ...
        var createTime: Long = DateEx.CurrentLong
    ...
    private suspend fun prepareGame(room: Room) {
        ...
        // 更新创建时间
        val newCreateTime = DateEx.CurrentLong
    ...
    private suspend fun endGame(room: Room) {
        ...
        // 计算时长
        val lastSubmitTime = DateEx.CurrentLong
        ...
        // 超过 60% 正确率的胜者存入数据库
        Coroutines.io {
            val isWinner1 = count1 > count2 || count1 == count2 && duration1 <= duration2
            ...
            if (winnerResult.count >= (LyricsSockets.QUESTION_COUNT * 0.6f).toInt()) {
                mysql.throwInsertSQLGeneratedKey("INSERT INTO game_alone_record (type, uid, question, answer, result) ${values(5)}",
