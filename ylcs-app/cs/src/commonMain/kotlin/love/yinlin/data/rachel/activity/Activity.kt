...
data class Activity(
	...
	override fun compareTo(other: Activity): Int {
		...
		val day1 = (this.ts?.let(DateEx.Formatter.standardDate::parse)?.toEpochDays() ?: Long.MAX_VALUE) - today
		val day2 = (other.ts?.let(DateEx.Formatter.standardDate::parse)?.toEpochDays() ?: Long.MAX_VALUE) - today
		return if (day1 < 0L) {
