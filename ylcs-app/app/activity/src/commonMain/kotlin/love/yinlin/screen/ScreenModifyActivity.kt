...
class ScreenModifyActivity(private val aid: Int) : Screen() {
    ...
    private suspend fun updateActivity() {
        ...
        DataSourceActivity.updateActivity(aid, newActivity) { oldActivity ->
            oldActivity.copy(
                ts = ts,
                tsInfo = tsInfo,
                location = location,
    ...
    private suspend fun updatePhoto(key: String, path: File) {
        slot.loading.open(content = "正在上传...") {
            DataSourceActivity.updateActivityPhoto(aid, key, path) { newPic, oldActivity ->
    ...
    private suspend fun deletePhoto(key: String) {
        slot.loading.open(content = "正在删除...") {
            DataSourceActivity.deleteActivityPhoto(aid, key) {
                photo = photo.toJson().Object.toMutableMap().let { map ->
                    map.remove(key)
                    map.toJson().to()
                }
    ...
    private suspend fun addPhotos(key: String, files: List<File>) {
        slot.loading.open(content = "正在上传...") {
            DataSourceActivity.addActivityPhotos(aid, key, files) { newPics, oldActivity ->
                photo = photo.toJson().Object.toMutableMap().let { map ->
    ...
    private suspend fun updatePhotos(key: String, index: Int) {
        app.picker.pickPicture()?.use { source ->
            ...
            slot.loading.open("正在更新...") {
                DataSourceActivity.updateActivityPhotos(aid, key, index, path) { newPic, oldActivity ->
                    photo = photo.toJson().Object.toMutableMap().let { map ->
    ...
    private suspend fun deletePhotos(key: String, index: Int) {
        slot.loading.open(content = "正在删除...") {
            DataSourceActivity.deleteActivityPhotos(aid, key, index) { oldActivity ->
                photo = photo.toJson().Object.toMutableMap().let { map ->
                    map[key] = map[key].Array.toMutableList().also { list ->
                        list.removeAt(index)
                    }.toJson()
