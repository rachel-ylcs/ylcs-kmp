...
class BlurVisualEffect() : VisualEffect {
  ...
  constructor(other: BlurVisualEffect) : this() {
  ...
  /**
   * The to apply to the blurred content.
   *
   * There are precedence rules to how this styling property is applied:
   *
   ...
   */
  var colorEffects: List<HazeColorEffect> = emptyList()
    get() {
  ...
  /**
   * The to use when Haze uses the fallback scrim functionality.
   *
   ...
   */
  var fallbackTint: HazeColorEffect = HazeColorEffect.Unspecified
    get() {
   ...
   * best-effort attempt.
   */
  var blurredEdgeTreatment: BlurredEdgeTreatment = HazeBlurDefaults.blurredEdgeTreatment
    set(value) {
      if (value != field) {
        field = value
   ...
   */
  var style: HazeBlurStyle = HazeBlurStyle.Unspecified
    set(value) {
      if (field != value) {
        onStyleChanged(field, value)
        field = value
