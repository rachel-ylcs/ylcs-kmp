...
class CharacterWanYouYinLi : Character() {
    override val info: CharacterInfo = CharacterInfo.WanYouYinLi
    ...
    override fun modifyResult(musicInfo: MusicInfo, config: RhymePlayConfig, block: Block<*>, result: BlockResult): RhymeSkillResult {
        ...
        val has4 = when (block) {
            is NoteBlock -> block.scaleIndex == 4
            is ContinuedBlock -> block.scaleIndex == 4
            is MultipleBlock -> 4 in block.scaleIndexs
        }
        return if (has4) {
            showText = "+${++count}"
