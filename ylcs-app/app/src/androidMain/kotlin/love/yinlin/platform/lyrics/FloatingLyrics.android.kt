...
actual class FloatingLyrics actual constructor(context: Context) {
    ...
    private val view = ComposeView(activity).apply {
        ...
        setContent { this@FloatingLyrics.Content() }
    }
    ...
    fun applyPermission(onResult: (Boolean) -> Unit) = catching {
        activity.activityResultRegistry.register(
            ...
            setData(android.net.Uri.fromParts(Scheme.Package.toString(), activity.packageName, null))
        })
    }

    fun attach() {
        if (canAttached && !isAttached) {
            val manager = activity.getSystemService(android.content.Context.WINDOW_SERVICE) as? WindowManager
    ...
    actual suspend fun init() {
        if (app.config.enabledFloatingLyrics) attach()
    }
